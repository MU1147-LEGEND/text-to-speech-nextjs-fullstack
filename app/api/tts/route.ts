...
export async function POST(req: Request) {
  ...
  if (!key || !region) return new Response("Missing Azure env vars", { status: 500 });
  ...
  const format = String(body?.format ?? "audio-16khz-128kbitrate-mono-mp3");
  ...
  if (!tokenRes.ok) return new Response("Token error", { status: 502 });
  ...
  if (!ttsRes.ok) return new Response("TTS error", { status: 502 });
